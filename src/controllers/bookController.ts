...
//     user?: { id: number }; // Adjust this based on your actual user model
// }
...
// // Update a book by ID in the database
// const updateBook = asyncWrqper(async (req: ExtendedRequest, res: Response, next: NextFunction) => {
//     const id = Number(req.params.id);
//     const { name, isbn, userId } = req.body;

//     // Check if the user is updating their own book
...
// // Delete a book by ID from the database
// const deleteBook = asyncWrqper(async (req: ExtendedRequest, res: Response, next: NextFunction) => {
//     const id = Number(req.params.id);

//     // Check if the user is deleting their own book
//     if (req.user && userId === req.user.id) {
//         const deletedRowCount = await Books.destroy({ where: { id } });
//         if (deletedRowCount === 0) {
//             return next(createCustomError(`No book with id: ${id} is found`, 404))
...
// // Rent a book
// const rentBook = asyncWrqper(async (req: ExtendedRequest, res: Response, next: NextFunction) => {
//     const bookId = Number(req.params.bookId);
//     const userId = Number(req.params.userId);
...
//     // Check if the user is renting their own book
//     if (req.user && userId === req.user.id) {
//         // Update book details to mark it as rented
//         await book.update({
//             userId, // ID of the customer who is renting the book
